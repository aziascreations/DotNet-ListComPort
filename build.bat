...
:: Used to list get a list of all DLLs used by the application to know which external licenses to use.
echo Building "Trimmed" targets...
dotnet publish "./NibblePoker.Application.ListComPort/NibblePoker.Application.ListComPort.csproj" --nologo -r win-x86   -c Release -o "./Builds/x86_trimmed"   -p:PublishTrimmed=true --self-contained true
dotnet publish "./NibblePoker.Application.ListComPort/NibblePoker.Application.ListComPort.csproj" --nologo -r win-x64   -c Release -o "./Builds/x64_trimmed"   -p:PublishTrimmed=true --self-contained true
dotnet publish "./NibblePoker.Application.ListComPort/NibblePoker.Application.ListComPort.csproj" --nologo -r win-arm   -c Release -o "./Builds/arm_trimmed"   -p:PublishTrimmed=true --self-contained true
dotnet publish "./NibblePoker.Application.ListComPort/NibblePoker.Application.ListComPort.csproj" --nologo -r win-arm64 -c Release -o "./Builds/arm64_trimmed" -p:PublishTrimmed=true --self-contained true
echo.
